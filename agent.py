...
from livekit.plugins import openai, silero, sarvam, elevenlabs, turn_detector
...
"""
    ...
    def on_user_speech_committed(ev):
        try:
            text = _extract_text(ev)
            # Ignore short transcripts (noise, breath, background sounds)
            if len(text.strip()) < 30:
                logger.info(f"Short transcript ignored ({len(text.strip())} chars): '{text.strip()}'")
                return
            _last_activity[0] = time.monotonic()
            if any(phrase in text for phrase in end_phrases):
                logger.info(f"Customer end phrase detected: '{text[:50]}' — hanging up in 3s")
                asyncio.create_task(_hang_up(3))
        except Exception as e:
